...
export function Header(props) {
  ...
  if (props.actions && props.data[colLength]?.name)
    data.push({ name: "actions", title: "", sort: false })

  // sort the columns
  function sort(index, cell) {
    if (!props.data[index].sort) return false
  ...
  return (
    <thead className={Style.thead}>
      <tr className="sticky top-0 z-22">
        {data.map((cell, index) => {
          // hide is_not_read field
          if (cell.name === "is_not_read") {
            return false
          }

          // hide
          if (props.hide?.includes(cell.name)) return false

          // show
          if (
            props.show &&
            !props.show.includes(cell.name) &&
            cell.name !== "actions"
          )
            return false
